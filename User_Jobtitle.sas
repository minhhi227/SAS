data sastest.zzsasusers_logins;
...
the maximum length for each variable.  */
 ...
 length LoginObjId UserId IdentId AuthDomId $ 17
        IdentType $ 32
        Name DispName Desc JobTitle uri uri2 uri3 AuthDomName $ 256;

/* The CALL MISSING routine initializes the output variables to missing values.  */

 call missing
(LoginObjId, UserId, IdentType, IdentId, Name, DispName, Desc, JobTitle, AuthDomId, AuthDomName);
...
Name, DisplayName and Desc attributes. */
	  ...
	  arc=metadata_getattr(uri2,"Title",JobTitle);
    end;
...
AuthenticationDomain object's Id and Name attributes. */
 ...
 call missing(LoginObjId, UserId, IdentType, IdentId, Name, DispName, Desc, JobTitle, AuthDomId, 
AuthDomName);
...
/* The KEEP statement specifies the variables to include in the output data set.  */
 ...
 keep LoginObjId UserId IdentType Name DispName Desc JobTitle AuthDomId AuthDomName; 
run;
